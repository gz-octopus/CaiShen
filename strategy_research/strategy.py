# -*- coding: utf-8 -*-
"""第一闭环策略：sh000001 上证指数 MA(10)/MA(30) 金叉择时。
...
- 资金管理：MM_FixedPercent(0.99) 全仓进出（空仓起步）。
  ...
  30 次全部成交且无警告。0.99 即「全仓」的落地形态。
- 无止损止盈（不配置 ST/TP）
- 交易假设：初始资金 100 万；TC_FixedA2017 + SP_FixedPercent(0.001)；
  信号次日开盘价成交（System 原生 buy_delay/sell_delay 行为）
- A 股规则：一字板延迟 + 停牌自然跳过（System 默认）；T+1 由次日开盘执行模型
  自动满足（买卖间隔必然 ≥1 个交易日），报告有显式声明
"""
...
def create_system(tm: hku.TradeManager, fast_n: int = 10, slow_n: int = 30,
    ...
    """构建 MA 金叉择时 System（参数固定，不调参）。

    :param tm: TradeManager 账户（须用 crtTM 以关键字参数构造：date/init_cash/cost_func/name）
