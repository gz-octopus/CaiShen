...
def main(
    ...
    with Api((IP, PORT)) as api:
        ...
        # for m in markets:
        #     mt = MarketType(m)
        #     market_enum = Api.Market(mt.int_value)
        #     I(m=m, mt=mt, int=mt.int_value, market_enum=market_enum)
        ...
        for market in markets:  # e.g., [Api.Market.SH, Api.Market.SZ]
            ...
            while done < stock_count:
                ...
                for stock_detail in stock_list:
                    ...
                    if stock_detail['股票代码'] == '603358':
                        I(code=code, code_Security_type=code.security_type)
        ...
        for short_code in stocks:
            ...
            # TODO: 后续可以把 simple_pytdx 中 api 相关的枚举和有用的方法迁移到 pytdx/tdxpy 中。
