...
Claude Code 配置（~/.claude.json → projects.D:/cortex.mcpServers）：
    "caishen": {
      "command": "D:/dev-environment/python312/python.exe",
...
if __name__ == "__main__":
    sys.modules['caishen_mcp.server'] = sys.modules['__main__']
...
# ── 启动时初始化 ──
# 注意：必须用绝对路径，Claude Code 的 cwd 配置不生效
_CONFIG_PATH = os.path.join(_PROJECT_ROOT, "config.yaml")
_tq_stdout_buffer = StringIO()
try:
    with redirect_stdout(_tq_stdout_buffer):
        _CFG, _DB_URL, _CONSOLE = init_tq_and_cache(_CONFIG_PATH)
...
from caishen_mcp.tools import cli_tools        # noqa: E402, F401
